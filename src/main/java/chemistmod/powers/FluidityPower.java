...
public class FluidityPower extends AbstractPower {
    public static final String POWER_ID = ChemistMod.makeId("FluidityPower");
    private static final PowerStrings POWER_STRINGS = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);

    public FluidityPower(AbstractCreature owner, int amount) {
    ...
    public void stackPower(int amount) {
        this.amount += amount;
        if (amount == 0) {
            AbstractDungeon.actionManager.addToTop(new RemoveSpecificPowerAction(this.owner, this.owner, FluidityPower.POWER_ID));
        }
    }
